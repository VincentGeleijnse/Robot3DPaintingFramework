...
class GDIndexedColorConverter
{
	...
	private function floydSteinbergDither($im, $width, $height, &$palette, $amount)
	{
		...
		for ($i = 0; $i < $height; $i++) {
			...
			for ($j = 0; $j < $width; $j++) {
				if ($i === 0 && $j === 0) {
					$color = $this->RGBtoLab($this->getRGBColorAt($im, $j, $i));
				} else {
					$color = $currentRowColorStorage[$j];
				}
				$closestColor = $this->getClosestColor(array('lab' => $color), $palette, 'lab');
				$closestColor = $closestColor['rgb'];

				if ($j < $width - 1) {
					if ($i === 0) {
						$currentRowColorStorage[$j + 1] = $this->RGBtoLab($this->getRGBColorAt($im, $j + 1, $i));
					}
				}
				if ($i < $height - 1) {
					if ($j === 0) {
						$nextRowColorStorage[$j] = $this->RGBtoLab($this->getRGBColorAt($im, $j, $i + 1));
					}
					if ($j < $width - 1) {
						$nextRowColorStorage[$j + 1] = $this->RGBtoLab($this->getRGBColorAt($im, $j + 1, $i + 1));
					}
				}
	 ...
	 */
	private function getClosestColor($pixel, &$palette, $mode = 'lab')
	{
		$closestColor;
		$closestDistance;
	...
	private function getRGBColorAt($im, $x, $y) {
		...
		cols = imagecolorsforindex($im, $index);
