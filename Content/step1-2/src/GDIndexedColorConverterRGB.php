...
class GDIndexedColorConverter
{
	...
	private function floydSteinbergDither($im, $width, $height, &$palette, $amount)
	{
		...
		for ($i = 0; $i < $height; $i++) {
			...
			for ($j = 0; $j < $width; $j++) {
				...
				$newColor = imagecolorallocate($newImage, $color[0], $color[1], $color[2]);
	...
	private function getClosestColor($pixel, &$palette, $mode = 'lab')
	{
		...
		foreach ($palette as $color) {
			...
			//print_r($pixel[$mode]);
			//print(strval($pixel[$mode][0]));
			//print(" ");
			//print(strval($pixel[$mode][1]));
			//print(" ");
			//print(strval($pixel[$mode][2]));
	...
	private function getRGBColorAt($im, $x, $y) {
		...
		return array($cols['red'], $cols['green'], $cols['blue']);
